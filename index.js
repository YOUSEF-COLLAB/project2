...
// 🤖 Prediction route using numerical sensor data
...
// 🧠 Face classification from base64 image
app.post('/api/classify-face', (req, res) => {
  const { image } = req.body;
  if (!image) return res.status(400).json({ message: 'No image provided' });

  PythonShell.run('models/model.py', { args: [image] }, (err, results) => {
    if (err || !results || results.length === 0) {
      return res.status(500).json({ message: 'Model error', error: err?.message || 'No output' });
    }

    const [prediction, alert] = results[0].split('|');
    res.json({ prediction, alert });
  });
});
